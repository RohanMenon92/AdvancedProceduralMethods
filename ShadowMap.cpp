...
bool ShadowMap::InitializeBuffers(ID3D11Device* device, int sampleCount, int qualityLevel)
{
	...
	result = shadowMapTexture->Initialize(device, ShadowConfig::SHADOWMAP_WIDTH, ShadowConfig::SHADOWMAP_HEIGHT, ShadowConfig::SCREEN_DEPTH, ShadowConfig::SCREEN_NEAR, sampleCount, qualityLevel);
	if (!result)
	{
