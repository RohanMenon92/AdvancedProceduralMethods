...
bool SkydomeShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"skydome_vs.hlsl", L"skydome_ps.hlsl");
	if (!result)
	{
		return false;
	}
...
bool SkydomeShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, Matrix worldMatrix, Matrix viewMatrix,
	Matrix projectionMatrix, Vector4 apexColor, Vector4 centerColor)
{
	bool result;
...
bool SkydomeShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, Matrix worldMatrix, Matrix viewMatrix,
	Matrix projectionMatrix, Vector4 apexColor, Vector4 centerColor)
{
	HRESULT result;
